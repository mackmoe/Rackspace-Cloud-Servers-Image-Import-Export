...
####### while loop to poll export until complete
getEXPprogress
...
getIMPprogress() {
  IMP_POLL=$(curl -s "https://$EXP_DC.images.api.rackspacecloud.com/v2/$CUSTOMERID/tasks/$IMP_TASKID" -X GET -H "X-Auth-Token: $TOKEN" -H "Content-Type: application/json")
